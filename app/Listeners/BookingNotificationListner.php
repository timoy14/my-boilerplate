...
class BookingNotificationListner
{
    /**
    ...
    public function handle($event)
    {
        ...
        $notification->message = $event->message;
        $notification->save();
        Mail::to('admin@alrapeh.com')->send(new NewBookingMail($event->booking_id));
        $tokens = Token::whereIn('user_id', [$event->owner_id, $event->customer_id])->get();
            ...
            if (!empty($token->firebase_token)) {
                ...
                $data = array("to" => $token->firebase_token,
                    ...
                    "data" => array("url" => "https://app.alrapeh.com/images/defaults/alrapeh.png",
                        ...
                        "booking_payment_id" => $event->booking_payment_id,
